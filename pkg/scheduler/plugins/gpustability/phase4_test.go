...
func phase4Mission(now time.Time) *spacev1.SpaceMission {
	...
	return &spacev1.SpaceMission{ObjectMeta: metav1.ObjectMeta{Name: "local-flow", Namespace: "missions", UID: types.UID("mission-uid"), Generation: 1}, Spec: spacev1.SpaceMissionSpec{MissionClass: "science", Priority: 500, StatePolicy: spacev1.PolicyStrict, RequiredCapabilities: []spacev1.CapabilityRequirement{{Class: "gpu", Quantity: 1, Architecture: "space-cuda"}}, Inputs: []spacev1.DataObject{{ID: "local-frame", SizeBytes: 1000, Locations: []spacev1.DataLocation{{Domain: spacev1.DomainReference{Name: "leo-a", ClusterID: "leo-a-cluster", OrbitClass: spacev1.OrbitLEO}}}}}, OutputSizeBytes: 1000, ResultDestinations: []spacev1.DataLocation{{Domain: spacev1.DomainReference{Name: "ground-a", ClusterID: "ground-a-cluster", OrbitClass: spacev1.OrbitGround}}}, ResultReturnRequired: true, Deadline: metav1.NewTime(now.Add(time.Hour)), ExpectedDurationSeconds: 60, MaximumDurationSeconds: 100, DurationUncertaintySecs: 20, SafetyMarginSeconds: 10, MaximumClockSkewSeconds: 2, Retry: spacev1.RetryPolicy{MaxAttempts: 2, AllowMigration: true, MaxConcurrentExecutions: 1}, Checkpoint: spacev1.CheckpointPolicy{Checkpointable: true}, WorkloadTemplate: v1.PodTemplateSpec{Spec: v1.PodSpec{Containers: []v1.Container{{Name: "processor", Image: "example.invalid/processor:v1", Resources: v1.ResourceRequirements{Requests: v1.ResourceList{"iluvatar.com/gpu": quantity}, Limits: v1.ResourceList{"iluvatar.com/gpu": quantity}}}}}}}}
}
