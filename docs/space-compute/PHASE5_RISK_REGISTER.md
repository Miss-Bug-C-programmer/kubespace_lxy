...
| R6 | Mitigated / hardware qualification open | Phase 9 adds dual-served v1beta1 canonical PhysicalDeviceInventory plus resource CPU/memory/storage/NUMA/trust/autonomy/energy fields, lossless conversion and stored-version migration | API owners | Keep conversion/rollback gates; qualify vendor population, physical allocation identity and telemetry accuracy under R4/R8 before claiming hardware traceability. |
| R7 | Resolved in CPU-only planning model | Phase 9 adds working memory/storage and minimum-bandwidth/maximum-RTT/maximum-loss fields and enforces them as non-relaxable planner hard constraints in strict/degraded/best-effort modes | API/planner | Retain boundary/conversion tests and validate deployment-specific resource/link reports before production use. |
...
| R9 | Medium | Phase 10 reduced the 5,000-target Collector one-shot result from the Phase 5 233.92 MB / 916.426 ms baseline to 191.332 MB / 474.468 ms (allocation bytes -18.2%, latency -48.2%), but no multi-hour live-endpoint soak or sustained API-throttling campaign has been run | Performance | Execute deployment-class soak with 1k+ live endpoints, sustained API throttling and heap/goroutine profiles; then set percentile SLOs from repeated measurements. |
| R10 | Resolved | Phase 8 adds hard pending-unique-key queue caps with readiness failure/saturation/oldest-age metrics, namespaced Mission ResourceQuota examples, cluster link/resource admission quotas, and bounded reporter QPS/burst | Controllers/operations | Keep limits sized below API-server/controller SLOs and retain API-throttling/soak qualification under R9. |
