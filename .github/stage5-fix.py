...
# Patch persistent dedupe API so receiver commits only after durable handling.
path=Path("contrib/space-compute/pkg/transport/spool.go")
text=path.read_text()
old='''func (d *DedupeStore) SeenOrRecord(id string,sequence int64,now time.Time)(bool,error){d.mu.Lock();defer d.mu.Unlock();d.prune(now);key:=fmt.Sprintf("%s#%d",id,sequence);if _,ok:=d.entries[key];ok{return true,nil};d.entries[key]=now.UTC();return false,writeAtomic(d.path,d.entries)}'''
new='''func (d *DedupeStore) Seen(id string, sequence int64, now time.Time) (bool, error) {\n\td.mu.Lock()\n\tdefer d.mu.Unlock()\n\td.prune(now)\n\t_, ok := d.entries[fmt.Sprintf("%s#%d", id, sequence)]\n\treturn ok, nil\n}\nfunc (d *DedupeStore) Record(id string, sequence int64, now time.Time) error {\n\td.mu.Lock()\n\tdefer d.mu.Unlock()\n\td.prune(now)\n\td.entries[fmt.Sprintf("%s#%d", id, sequence)] = now.UTC()\n\treturn writeAtomic(d.path, d.entries)\n}\nfunc (d *DedupeStore) SeenOrRecord(id string, sequence int64, now time.Time) (bool, error) {\n\tseen, err := d.Seen(id, sequence, now)\n\tif err != nil || seen { return seen, err }\n\treturn false, d.Record(id, sequence, now)\n}'''
if text.count(old)!=1: raise SystemExit("dedupe marker mismatch")
path.write_text(text.replace(old,new,1))
print("stage5 transport compile fix applied")
